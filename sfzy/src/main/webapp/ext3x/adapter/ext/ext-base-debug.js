...
(function() {
	...
	Ext.apply(Ext, {
		...
		/**
		 * HIGHLY EXPERIMENTAL True to force css based border-box model override
		 * and turning off javascript based adjustments. This is a runtime
		 * configuration and must be set before onReady.
		 * 
		 * @type Boolean
		 */
		...
		/**
		 * EXPERIMENTAL - True to cascade listener removal to child elements
		 * when an element is removed. Currently not optimized for performance.
		 * 
		 * @type Boolean
		 */
		 ...
		 * 
		 * @param {Mixed}
		 *            el (optional) The element to generate an id for
		 * @param {String}
		 *            prefix (optional) Id prefix (defaults "ext-gen")
		...
		/**
		 * <p>
		 * Extends one class to create a subclass and optionally overrides
		 * members with the passed literal. This method also adds the function
		 * "override()" to the subclass that can be used to override members of
		 * the class.
		 * </p>
		 ...
		 * 
		 * <p>
		 * This function also supports a 3-argument call in which the subclass's
		 * constructor is passed as an argument. In this form, the parameters
		 * are as follows:
		 * </p>
		 * <div class="mdetail-params">
		 * <ul>
		 * <li><code>subclass</code> : Function <div class="sub-desc">The
		 * subclass constructor.</div></li>
		 * <li><code>superclass</code> : Function <div class="sub-desc">The
		 * constructor of class being extended</div></li>
		 * <li><code>overrides</code> : Object <div class="sub-desc">A
		 * literal with members which are copied into the subclass's prototype,
		 * and are therefore shared among all instances of the new class.</div></li>
		 * </ul>
		 * </div>
		 * 
		 * @param {Function}
		 *            superclass The constructor of class being extended.
		 * @param {Object}
		 *            overrides
		 *            <p>
		 *            A literal with members which are copied into the
		 *            subclass's prototype, and are therefore shared between all
		 *            instances of the new class.
		 *            </p>
		 *            <p>
		 *            This may contain a special member named
		 *            <tt><b>constructor</b></tt>. This is used to define
		 *            the constructor of the new class, and is returned. If this
		 *            property is <i>not</i> specified, a constructor is
		 *            generated and returned which just calls the superclass's
		 *            constructor passing on its parameters.
		 *            </p>
		 *            <p>
		 *            <b>It is essential that you call the superclass
		 *            constructor in any provided constructor. See example code.</b>
		 *            </p>
		 * @return {Function} The subclass constructor from the
		 *         <code>overrides</code> parameter, or a generated one if not
		 *         provided.
		 */
		extend : function() {
			// inline overrides
		 ...
		 * overwriting any existing methods with the same name. Usage:
		 * 
		 * <pre><code>
		 ...
		 * Ext.urlEncode({foo: 1, bar: 2}); would return "foo=1&bar=2".
		 ...
		 *            first argument to <code>Ext.each</code>.</div></li>
		 *            </ul>
		 *            </div>
		 * @param {Object}
		 ...
		 * Ext.Element. Optional 'strict' flag is needed for IE since it can
		...
		/**
		 * <p>
		 * Removes this element from the document, removes all DOM event
		 * listeners, and deletes the cache reference. All DOM event listeners
		 * are removed from this element. If
		 * {@link Ext#enableNestedListenerRemoval} is <code>true</code>, then
		 * DOM event listeners are also removed from all child nodes. The body
		 * node will be ignored if passed in.
		 * </p>
		 * 
		 * @param {HTMLElement}
		...
		removeNode : isIE && !isIE8 ? function() {
			...
			return function(n) {
				if (n && n.tagName != 'BODY') {
					(Ext.enableNestedListenerRemoval) ? Ext.EventManager
							.purgeElement(n, true) : Ext.EventManager
							.removeAll(n);
			...
			if (n && n.parentNode && n.tagName != 'BODY') {
				(Ext.enableNestedListenerRemoval) ? Ext.EventManager
						.purgeElement(n, true) : Ext.EventManager.removeAll(n);
	...
	/**
	 * Creates namespaces to be used for scoping variables and classes so that
	 * they are not global. Specifying the last node of a namespace implicitly
	 * creates all other nodes. Usage:
	 * 
	 * <pre><code>
	 * 	Ext.namespace('Company', 'Company.data');
	 * 	Ext.namespace('Company.data'); // equivalent and preferable to above syntax
...
Ext.apply(Function.prototype, {
			/**
			 * Creates an interceptor function. The passed function is called
			 * before the original one. If it returns false, the original one is
			 * not called. The resulting function returns the results of the
			 * original function. The passed function is called with the
			 * parameters of the original function. Example usage:
			 * 
			 * <pre><code>
			 ...
			 *            <b>If omitted, defaults to the scope in which the
			 *            original function is called or the browser window.</b>
			...
			/**
			 * Creates a callback that passes arguments[0], arguments[1],
			 * arguments[2], ... Call directly on any function. Example:
			 * <code>myFunction.createCallback(arg1, arg2)</code> Will create
			 * a function that is bound to those 2 args. <b>If a specific scope
			 * is required in the callback, use {@link #createDelegate} instead.</b>
			 * The function returned by createCallback always executes in the
			 * window scope.
			 * <p>
			 * This method is required when you want to pass arguments to a
			 * callback function. If no arguments are needed, you can simply
			 * pass a reference to the function as a callback (e.g., callback:
			 * myFn). However, if you tried to pass a function with arguments
			 * (e.g., callback: myFn(arg1, arg2)) the function would simply
			 * execute immediately when the code is parsed. Example usage:
			 * 
			 * <pre><code>
			 * var sayHi = function(name) {
			 * 	alert('Hi, ' + name);
			...
			/**
			 * Creates a delegate (callback) that sets the scope to obj. Call
			 * directly on any function. Example:
			 * <code>this.myFunction.createDelegate(this, [arg1, arg2])</code>
			 * Will create a function that is automatically scoped to obj so
			 * that the <tt>this</tt> variable inside the callback points to
			 * obj. Example usage:
			 * 
			 * <pre><code>
			 * var sayHi = function(name) {
			 ...
			 * @param {Object}
			 *            scope (optional) The scope (<code><b>this</b></code>
			 *            reference) in which the function is executed. <b>If
			 *            omitted, defaults to the browser window.</b>
			 * @param {Array}
			 *            args (optional) Overrides arguments for the call.
			 *            (Defaults to the arguments passed by the caller)
			 * @param {Boolean/Number}
			 *            appendArgs (optional) if True args are appended to
			 *            call args instead of overriding, if a number the args
			 ...
			 * optionally in a specific scope. Example usage:
			 * 
			 * <pre><code>
			 * var sayHi = function(name) {
			 * 	alert('Hi, ' + name);
			 ...
			 * 
			 * @param {Number}
			 *            millis The number of milliseconds for the setTimeout
			 *            call (if less than or equal to 0 the function is
			 ...
			 *            call args instead of overriding, if a number the args
			 *            are inserted at the specified position
			 * @return {Number} The timeout id that can be used with
			 *         clearTimeout
			 */
...
/**
 * @class Ext.util.TaskRunner Provides the ability to execute one or more
 *        arbitrary tasks in a multithreaded manner. Generally, you can use the
 *        singleton {@link Ext.TaskMgr} instead, but if needed, you can create
...
/**
 * @class Ext.TaskMgr
 * @extends Ext.util.TaskRunner A static {@link Ext.util.TaskRunner} instance
 *          that can be used to start and stop arbitrary tasks. See
 *          {@link Ext.util.TaskRunner} for supported methods and task config
 *          properties.
 * 
 * <pre><code>
 * // Start a simple clock task that updates a div once per second
 ...
 * 
 * <p>
 *          See the {@link #start} method for details about how to configure a
 *          task object.
 *          </p>
 * @singleton
 */
...
(function() {
	...
	(function() {
		...
		Ext.lib.Dom = {
			...
			getViewHeight : function(full) {
				return full ? this.getDocumentHeight() : this
						.getViewportHeight();
			},
			...
			getDocumentHeight : function() {
				return MAX(!isCSS1
								...
								: doc.documentElement.scrollHeight, this
								.getViewportHeight());
			},
			...
			getDocumentWidth : function() {
				return MAX(!isCSS1
								...
								: doc.documentElement.scrollWidth, this
								.getViewportWidth());
			},
					...
					if (el.getBoundingClientRect) {
						...
						while (p) {
							...
							hasAbsolute = hasAbsolute
									|| pe.isStyle("position", "absolute");

							if (Ext.isGecko) {
								y += bt = PARSEINT(pe
												.getStyle("borderTopWidth"), 10)
										|| 0;
								x += bl = PARSEINT(pe
												.getStyle("borderLeftWidth"),
										10)
										|| 0;

								if (p != el
										&& !pe.isStyle('overflow', 'visible')) {
									x += bl;
									y += bt;
								}
							}
						...
						if (Ext.isGecko && !hasAbsolute) {
							dbd = fly(bd);
							x += PARSEINT(dbd.getStyle("borderLeftWidth"), 10)
									|| 0;
							y += PARSEINT(dbd.getStyle("borderTopWidth"), 10)
									|| 0;
						}
	...
	Ext.lib.Event = function() {
		var loadComplete = false, unloadListeners = {}, retryCount = 0, onAvailStack = [], _interval, locked = false, win = window, doc = document,
				...
				for (i = 0; i < onAvailStack.length; ++i) {
					...
					if (v && (element = doc.getElementById(v.id))) {
						if (!v.checkReady || loadComplete
								|| element.nextSibling || (doc && doc.body)) {
							override = v.override;
		...
		var pub = {
			...
			resolveTextNode : Ext.isGecko ? function(node) {
				...
				if (s == '[xpconnect wrapped native prototype]'
						|| s == '[object XULElement]') {
					return;
				}
			...
			_unload : function(e) {
				...
				for (id in unloadListeners) {
					...
					for (i = 0, len = ul.length; i < len; i++) {
						...
						if (v) {
							try {
								scope = v[ADJ_SCOPE] ? (v[ADJ_SCOPE] === true
										? v[OBJ]
										: v[ADJ_SCOPE]) : win;
	...
	Ext.lib.Ajax = function() {
		var activeX = ['Msxml2.XMLHTTP.6.0', 'Msxml2.XMLHTTP.3.0',
				'Msxml2.XMLHTTP'], CONTENTTYPE = 'Content-Type';
		...
		function createResponseObject(o, callbackArg) {
			...
			return {
				tId : o.tId,
				// Normalize the status and statusText when IE returns 1223, see
				// the above link.
				...
				if (options) {
					...
					if (xmlData || jsonData) {
						...
						if (!hs || !hs[CONTENTTYPE]) {
							initHeader(CONTENTTYPE, xmlData
											? 'text/xml'
											: 'application/json');
						}
						data = xmlData
	...
	(function() {
		var EXTLIB = Ext.lib, noNegatives = /width|height|opacity|padding/i, offsetAttribute = /^((width|height)|(top|left))$/, defaultUnit = /width|height|top$|bottom$|left$|right$/i, offsetUnit = /\d+(em|%|en|ex|pt|in|cm|mm|pc)$/i, isset = function(
				v) {
			return typeof v !== 'undefined';
		}, now = function() {
			return new Date();
		...
		EXTLIB.AnimBase.prototype = {
			doMethod : function(attr, start, end) {
				var me = this;
				return me.method(me.curFrame, start, end - start,
						me.totalFrames);
			},
			...
			getAttr : function(attr) {
				var el = Ext.fly(this.el), val = el.getStyle(attr), a = offsetAttribute
						.exec(attr)
						|| [];
			...
			init : function(el, attributes, duration, method) {
				...
				var onTween = function() {
					...
					var ra = me.runAttrs;
					for (var attr in ra) {
						this.setAttr(attr, me.doMethod(attr, ra[attr].start,
										ra[attr].end), ra[attr].unit);
					}
				...
				(me._onComplete = new Ext.util.Event(me))
		...
		Ext.lib.AnimMgr = new function() {
			...
			var correctFrame = function(tween) {
				var frames = tween.totalFrames, frame = tween.curFrame, duration = tween.duration, expected = (frame
						* duration * 1000 / frames), elapsed = (now() - tween.startTime), tweak = 0;
		...
		(function() {
			EXTLIB.Motion = function(el, attributes, duration, method) {
				if (el) {
					EXTLIB.Motion.superclass.constructor.call(this, el,
			...
			Ext.apply(EXTLIB.Motion.prototype, {
				...
				getAttr : function(attr) {
					...
					return pointsRe.test(attr) ? [getAttr.call(me, 'left'),
							getAttr.call(me, 'top')] : getAttr.call(me, attr);
				},

				doMethod : function(attr, start, end) {
					var me = this;

					return pointsRe.test(attr) ? EXTLIB.Bezier.getPosition(
							me.runAttrs[attr], me.method(me.curFrame, 0, 100,
									me.totalFrames)
									/ 100) : superclass.doMethod.call(me, attr,
							start, end);
				},
				...
				setRunAttr : function(attr) {
					if (pointsRe.test(attr)) {
						...
						var me = this, el = this.el, points = this.attributes.points, control = points.control
								|| [], from = points.from, to = points.to, by = points.by, DOM = EXTLIB.Dom, start, i, end, len, ra;

						if (control.length > 0 && !Ext.isArray(control[0])) {
							control = [control];
						} else {
							/*
							 * var tmp = []; for (i = 0,len = control.length; i <
							 * len; ++i) { tmp[i] = control[i]; } control = tmp;
							 */
						}
						...
						if (isset(to)) {
							...
							for (i = 0, len = control.length; i < len; ++i) {
								control[i] = translateValues.call(me,
										control[i], start);
							}
						} else if (isset(by)) {
	...
	(function() {
		...
		Ext.apply(EXTLIB.Easing, {
					...
					elasticIn : function(t, b, c, d, a, p) {
						...
						return -(a * pow(2, 10 * (t -= 1)) * sin((t * d - s)
								* (2 * pi) / p))
								+ b;
					...
					elasticBoth : function(t, b, c, d, a, p) {
						...
						return t < 1 ? -.5
								* (a * pow(2, 10 * (t -= 1)) * sin((t * d - s)
										* (2 * pi) / p)) + b : a
								* pow(2, -10 * (t -= 1))
								* sin((t * d - s) * (2 * pi) / p) * .5 + c + b;
					},
					...
					bounceBoth : function(t, b, c, d) {
						return (t < d / 2) ? EXTLIB.Easing.bounceIn(t * 2, 0,
								c, d)
								* .5 + b : EXTLIB.Easing.bounceOut(t * 2 - d,
								0, c, d)
								* .5 + c * .5 + b;
					}
				});
	...
	(function() {
		...
		EXTLIB.Anim.color = function(el, args, duration, easing, cb, scope) {
			return EXTLIB.Anim.run(el, args, duration, easing, cb, scope,
					EXTLIB.ColorAnim);
		};

		EXTLIB.ColorAnim = function(el, attributes, duration, method) {
			EXTLIB.ColorAnim.superclass.constructor.call(this, el, attributes,
					duration, method);
		};

		Ext.extend(EXTLIB.ColorAnim, EXTLIB.AnimBase);

		var superclass = EXTLIB.ColorAnim.superclass, colorRE = /color$/i, transparentRE = /^transparent|rgba\(0, 0, 0, 0\)$/, rgbRE = /^rgb\(([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\)$/i, hexRE = /^#?([0-9A-F]{2})([0-9A-F]{2})([0-9A-F]{2})$/i, hex3RE = /^#?([0-9A-F]{1})([0-9A-F]{1})([0-9A-F]{1})$/i, isset = function(
				v) {
			return typeof v !== 'undefined';
		};
		...
		Ext.apply(EXTLIB.ColorAnim.prototype, {
			getAttr : function(attr) {
				...
				if (colorRE.test(attr)) {
					while (el
							&& transparentRE.test(val = Ext.fly(el)
									.getStyle(attr))) {
						el = el.parentNode;
						val = "fff";
					}
			...
			doMethod : function(attr, start, end) {
				...
				if (colorRE.test(attr)) {
					...
					val = 'rgb(' + floor(val[0]) + ',' + floor(val[1]) + ','
							+ floor(val[2]) + ')';
				} else {
	...
	(function() {
		...
		EXTLIB.Scroll = function(el, attributes, duration, method) {
			if (el) {
				EXTLIB.Scroll.superclass.constructor.call(this, el, attributes,
						duration, method);
			}
		};
		...
		Ext.apply(EXTLIB.Scroll.prototype, {
			...
			doMethod : function(attr, start, end) {
				...
				if (attr == SCROLL) {
					val = [
							me.method(curFrame, start[0], end[0] - start[0],
									totalFrames),
							me.method(curFrame, start[1], end[1] - start[1],
									totalFrames)];
				} else {
					val = superclass.doMethod.call(me, attr, start, end);
				}
